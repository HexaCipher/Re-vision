  ...
  return (
    <PageTransition className="min-h-screen" style={{ fontFamily: "var(--font-inter), sans-serif" }}>
      ...
      <nav className="fixed top-0 inset-x-0 z-50 flex items-center justify-between px-4 sm:px-6 lg:px-10 py-3 sm:py-4 border-b border-white/5 bg-black/40 backdrop-blur-2xl">
        ...
        >
          <Link href="/dashboard">
            <Button variant="ghost" className="text-slate-400 hover:text-white hover:bg-white/5 h-9 sm:h-11 px-3 sm:px-6 rounded-lg sm:rounded-xl text-sm sm:text-base">
      ...
      <div className="container mx-auto px-4 sm:px-6 pt-24 sm:pt-28 pb-8 sm:pb-12">
        <div className="max-w-4xl mx-auto">
          ...
          <FadeIn className="mb-6 sm:mb-10">
            <p className="text-sm sm:text-base text-indigo-400 font-semibold tracking-widest uppercase mb-3 sm:mb-4">Answer Review</p>
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  <Badge className="bg-indigo-500/15 text-indigo-400 border-indigo-500/25 font-medium text-xs sm:text-sm">
                    {quiz.subject}
                  </Badge>
              ...
              <div className="flex items-center gap-2 sm:gap-3 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-white/[0.02] border border-white/10 w-fit">
                <span className="text-slate-400 text-sm sm:text-base">Score:</span>
                <span className={`text-xl sm:text-2xl font-bold ${percentage >= 70 ? 'text-emerald-400' : percentage >= 50 ? 'text-amber-400' : 'text-red-400'}`}>
                  {correctCount}/{quiz.questions.length}
                </span>
                <span className="text-slate-500 text-sm sm:text-base">({percentage}%)</span>
              </div>
            </div>
          </FadeIn>
                ...
                <StaggerItem key={question.id}>
                  ...
                  >
                    ...
                    <div className="p-4 sm:p-6 md:p-8">
                      ...
                      {/* Answers */}
                      <div className="space-y-3 sm:space-y-4 mt-4 sm:mt-6 ml-0 sm:ml-[52px] md:ml-[76px]">
                        {/* User's Answer */}
                        <div>
                          <p className="text-xs sm:text-sm text-slate-500 mb-1.5 sm:mb-2 font-medium">Your Answer</p>
                          <div
                        ...
                        {!correct && (
                          <div>
                            <p className="text-xs sm:text-sm text-slate-500 mb-1.5 sm:mb-2 font-medium">
                              Correct Answer
                            </p>
                            <div className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-emerald-500/10 border border-emerald-500/25">
                        ...
                        {question.explanation && (
                          <div>
                            ...
                            <div className="p-3 sm:p-4 rounded-lg sm:rounded-xl bg-white/[0.02] border border-white/10">
                              <p className="text-sm sm:text-base text-slate-300 leading-relaxed">{question.explanation}</p>
                            </div>
                          </div>
                        )}
                      </div>
          ...
          {/* Actions */}
          <FadeIn delay={0.3} className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 sm:mt-10">
            <Link href={`/quiz/${quiz.id}/take`} className="flex-1">
              <Button className="w-full bg-white text-slate-950 hover:bg-slate-100 h-12 sm:h-14 rounded-xl text-sm sm:text-base font-semibold">
                <RotateCcw className="w-4 h-4 sm:w-5 sm:h-5 mr-2" />
                Retry Quiz
              </Button>
            </Link>
            <Link href="/dashboard" className="flex-1">
              <Button variant="outline" className="w-full h-12 sm:h-14 rounded-xl text-sm sm:text-base border-white/10 hover:bg-white/5 hover:border-white/20">
